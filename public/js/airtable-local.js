...
console.log(data)
      ...
      updateTable3(finalRecordsToShow); // Update the table with these records
...
// This event listener is specifically for the dropdown with ID 'myDropdown3'
document.querySelector('#myDropdown3').addEventListener('change', function(event) {
  if (event.target.type === 'checkbox') {
    // Update selectedLocals only within this dropdown
    selectedLocals = Array.from(this.querySelectorAll('.multipleSelectionOption input[type=checkbox]:checked'))
                            .map(checkbox => checkbox.value);
    // Call the function to update the table based on the newly selected Locals
    updateTable3BasedOnLocals();
  }
});

  function updateTable3BasedOnLocals() {
  fetch(
    "https://api.airtable.com/v0/appFafNlx9CAnh6Ze/Trades?view=Grid%20view",
    {
      headers: {
    ...
    .then((response) => response.json())
    .then((data) => {
      // Filter records by selected Locals first
      const filteredRecords = data.records.filter(
        (record) =>
          selectedLocals.length === 0 ||
          selectedLocals.includes(record.fields.Local)
      );
      ...
      updateTable3(finalRecordsToShow); // Use this function to update the table
    })
    .catch((error) => console.error("Error fetching data: ", error));
    console.log("Selected Locals for updateTable3BasedOnLocals:", selectedLocals);
  }

function updateTable3(records) {
    const tbody = document.querySelector(".orderbook-container3 .orderbook-table2 tbody");
